...
export default function ActivityStream({ onViewMore }: ActivityStreamProps) {
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-200 w-full max-w-full">
      ...
      <div className="space-y-4">
        {activities.map((activity, idx) => (
          <button
            key={idx}
            className="
              w-full flex gap-3 text-left p-3 rounded-lg 
          ...
          >
            {/* Avatar bubble */}
            <div
              className={`w-10 h-10 rounded-full ${activity.image} flex-shrink-0`}
            ></div>
            ...
            <div className="flex-1 min-w-[200px]">
              ...
              <div className="flex items-start sm:items-center flex-wrap gap-2">
                ...
                <p className="text-gray-700 text-xs break-all">{activity.action}</p>
              </div>
            </div>
          </button>
        ))}
      ...
      <button
        onClick={onViewMore}
        className="
          w-full mt-4 text-blue-600 hover:text-blue-700 text-sm font-medium 
